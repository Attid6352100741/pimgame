...
function Games3() {
    ...
    const handleChange = (choices, setChoice, setShuffledChoices) => (event) => {
        ...
        setChoice((prevChoice) => {
            localStorage.setItem(`game3Choice1_${user.id}`, prevChoice === choice1 ? selectedChoice : choice1 || '');
            localStorage.setItem(`game3Choice2_${user.id}`, prevChoice === choice2 ? selectedChoice : choice2 || '');
            localStorage.setItem(`game3Choice3_${user.id}`, prevChoice === choice3 ? selectedChoice : choice3 || '');
            localStorage.setItem(`game3Choice4_${user.id}`, prevChoice === choice4 ? selectedChoice : choice4 || '');
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            const storedScore = localStorage.getItem(`game3Score_${userId}`);
            const storedTestHistory = localStorage.getItem(`testHistory_${userId}`);
            const storedRound = localStorage.getItem(`round_course3_${userId}`);
            ...
            const storedChoice1 = localStorage.getItem(`game3Choice1_${userId}`);
            const storedChoice2 = localStorage.getItem(`game3Choice2_${userId}`);
            const storedChoice3 = localStorage.getItem(`game3Choice3_${userId}`);
            const storedChoice4 = localStorage.getItem(`game3Choice4_${userId}`);
    ...
    const handleClose = (confirmed) => {
        ...
        if (confirmed) {
            setSubmitted(true);
            checkAnswers();
            ["game3Choice1", "game3Choice2", "game3Choice3", "game3Choice4"].forEach((choiceKey) => {
    ...
    const checkAnswers = () => {
        ...
        localStorage.removeItem(`game3Score_${user.id}`);
        localStorage.setItem(`game3Score_${user.id}`, newScore.toString());
        localStorage.setItem(`testHistory_${user.id}`, JSON.stringify(updatedTestHistory));
    
        if (round < 5) {
            setRound((prevRound) => prevRound + 1);
            localStorage.setItem(`round_course3_${user.id}`, (round + 1).toString());
        }
    };
    ...
    return (
        <div>
            ...
            <Container component="main" maxWidth="xs">
                ...
                >
                    <h3 style={{ marginBottom: '10%' }}>English Word Game 3</h3>
